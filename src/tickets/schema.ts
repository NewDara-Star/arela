 ...
 */
export function validateTicket(ticket: unknown): {
  valid: boolean;
  errors: string[];
} {
  const errors: string[] = [];
  ...
  if (t.agent && !["codex", "claude", "deepseek", "ollama", "cascade"].includes(String(t.agent))) {
    errors.push(
      `Invalid agent: ${t.agent} (must be one of: codex, claude, deepseek, ollama, cascade)`,
    );
  }
  ...
  if (t.estimatedTime && !/^\d+[mhd]$/.test(String(t.estimatedTime))) {
    errors.push(
      `Invalid estimatedTime format: ${t.estimatedTime} (expected: e.g., 30m, 2h, 1d)`,
    );
  }
