    ...
    server.registerTool(
        ...
        async ({ content, mode }) => {
            ...
            if (content.trim().startsWith("{")) {
                // Test Tools
            case "arela_test_generate": {
                    const { prdPath } = args;
                    if (!prdPath) throw new Error("prdPath is required");
                    const result = await test.generateTests(projectRoot, prdPath as string);
                    return {
                        content: [{
                            type: "text",
            ...
            case "arela_test_run": {
                    const { featurePath } = args;
                    if (!featurePath) throw new Error("featurePath is required");
                    const result = await test.runTest(projectRoot, featurePath as string);
                    return {
                        content: [{
                            type: "text",
                            text: JSON.stringify(result, null, 2)
