...
export class SliceExtractor {
  ...
  private async commitSlices(slices: SliceExtractionPlan[], cwd: string): Promise<number> {
    ...
    for (const slice of slices) {
      try {
        const stagedFiles = slice.sourceFiles.map(f => f.destination);
        await this.gitManager.stageFiles(stagedFiles, cwd);
        await this.gitManager.commitSlice(slice, stagedFiles, cwd);
        commitCount++;
        this.log(`  ✅ Committed: Extract ${slice.name} slice`);
      } catch (error) {
        this.log(
          `  ⚠️ Failed to commit ${slice.name}: ${
