...
import type { DoctorViolation } from "./loaders.js";

type AgentName = "codex" | "claude";
type TicketPriority = "low" | "medium" | "high";
type TicketComplexity = "simple" | "medium" | "complex";
...
export async function generateTicketsFromViolations(opts: AutoTicketOptions): Promise<GeneratedTicket[]> {
...
function buildTicketGroups(violations: DoctorViolation[]): TicketGroup[] {
...
function getGroupKey(violation: DoctorViolation): string {
  const basis = violation.ruleId?.toLowerCase() ?? normalizeText(violation.message);
  return `${violation.source}:${basis}`;
}
...
function classifyViolation(violation: DoctorViolation): {
  agent: AgentName;
  complexity: TicketComplexity;
  priority: TicketPriority;
  estimate: string;
  ...
  if (violation.source === "workflow" || COMPLEX_KEYWORDS.some((pattern) => pattern.test(text))) {
...
function describeViolation(violation: DoctorViolation): { title: string; summary: string } {
  const base = violation.message.split(/\r?\n/)[0]?.trim() || "Resolve doctor violation";
...
function renderTicket(id: string, group: TicketGroup, cwd: string): string {
  ...
  return [
    ...
    "",
    `- Files: ${fileLine}`,
    `- Source: Doctor ${group.violations[0]?.source ?? "rule"}`,
    "",
    "## Violations",
    "",
    ...
    "- [ ] `npx arela doctor` passes without this issue",
    "- [ ] Relevant tests cover the regression",
    "",
  ].join("\n");
}

function formatViolationLine(index: number, violation: DoctorViolation, cwd: string): string {
  ...
  const detail = violation.message || violation.raw;
...
function buildFileList(violations: DoctorViolation[]): string[] {
